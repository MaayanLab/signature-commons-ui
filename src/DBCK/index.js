...
export default class DBCK extends React.Component {
  ...
  componentDidMount() {
    (async () => {
      this.setState({
        library_dbck: await fetch_meta('/libraries/dbck', {filter: JSON.stringify({limit: 10})}),
      })
    })();
    (async () => {
      this.setState({
        signature_dbck: await fetch_meta('/signatures/dbck', {filter: JSON.stringify({limit: 10})}),
      })
    })();
    (async () => {
      this.setState({
        entity_dbck: await fetch_meta('/entities/dbck', {filter: JSON.stringify({limit: 10})}),
      })
    })();
  }
  render() {
